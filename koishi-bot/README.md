...
插件采用独立版本号，当前版本为 `0.3.8`，不会跟随 Android 应用版本同步递增。

## 当前功能

- `加入排队`：使用当前 QQ 绑定的玩家资料选择机台并创建线上登记；默认偏好为“每次询问”时，还会询问本次游玩偏好。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.8.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.3.8.tgz
```
