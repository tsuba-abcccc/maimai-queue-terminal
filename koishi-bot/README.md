...
插件采用独立版本号，当前版本为 `0.3.3`，不会跟随 Android 应用版本同步递增。
...
- `排队通知`：查看个人通知总开关和五个分项；可以分别调整队列状态、游玩位置、线上登记与签到、暂缓暂离未到场、机台及营业状态通知。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.3.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.3.3.tgz
```
