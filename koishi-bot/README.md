...
插件采用独立版本号，当前版本为 `0.1.4`，不会跟随 Android 应用版本同步递增。
...
队列输出中的中点两侧不留空格，无等待登记时不额外显示空状态；通知固定以“【排队通知】”开头并空一行。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.1.4.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.1.4.tgz
```
...
- `notificationEnabled`：是否发送本人相关的私信通知，默认开启。
