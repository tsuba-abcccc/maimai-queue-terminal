...
插件采用独立版本号，当前版本为 `0.3.13`，不会跟随 Android 应用版本同步递增。
...
- `查看队列`：按现场分组查询机台 A 至 J 中实际配置机台的游玩位置、全部等待位置、登记数量和登记状态。机台停止使用时仍会显示已经保留的登记顺序。
- `查询人数`：汇总当前登记总数，并分别显示每台机台的登记数和新登记预计等待时间；停机、离线或关闭登记时会改为对应状态说明。
...
选择机台时可直接回复列表中的机台字母，例如 `A`、`B`、`C`，直至 `J`，小写同样有效；也可回复列表括号中的机台备注。只会列出现场实际配置的机台，备注只有在能够唯一对应一台机台时才会生效。
...
`加入排队`、`我的排队`、当前登记操作、`我的资料`和三项资料修改命令只允许在 OneBot 私聊中使用，群聊中不会返回个人资料或个人状态。`查看队列`和`查询人数`只读取公开队列，可以在群聊中使用。
...
3. 机厅安装的是具有同步能力的 `terminal` 变体，应用内“与服务端同步”和“QQ Bot 联动”均已开启，并且已经成功上传队列和玩家资料。返回 `503 QQ Bot 联动已关闭` 时需要在终端开启联动；其他 `503` 表示服务端令牌配置无效。纯本地 `local` 变体不会连接服务器。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.13.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：
