...
插件采用独立版本号，当前版本为 `0.3.6`，不会跟随 Android 应用版本同步递增。
...
选择机台时可直接回复机台字母，例如 `A`、`B`，小写同样有效；也可回复列表括号中的机台备注。备注只有在能够唯一对应一台机台时才会生效。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.6.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.3.6.tgz
```
