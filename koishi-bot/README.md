...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.2.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.3.2.tgz
```
