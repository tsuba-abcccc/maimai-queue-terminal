...
插件采用独立版本号，当前版本为 `0.3.7`，不会跟随 Android 应用版本同步递增。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.7.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.3.7.tgz
```
