...
插件采用独立版本号，当前版本为 `0.2.1`，不会跟随 Android 应用版本同步递增。
...
通过 Bot 创建的是“线上登记”。玩家到达现场后，必须在终端点击自己的登记并选择“已到场”完成签到；签到前不会进入游玩位置，也不会影响其他玩家的等待时间估算。待签到登记在 Bot 中只能退出排队，其他操作会由插件、服务器和终端分别校验并拒绝。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.2.0.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.2.0.tgz
```
