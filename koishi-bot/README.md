...
插件采用独立版本号，当前版本为 `0.3.4`，不会跟随 Android 应用版本同步递增。

## 当前功能

- `加入排队`：使用当前 QQ 绑定的玩家资料选择机台并创建线上登记；默认偏好为“每次询问”时，还会询问本次游玩偏好。
- `我的排队`：查询当前 QQ 对应登记的位置、预计等待时间、本次偏好、暂缓一轮、暂时离开和未到场状态。终端在线时，游玩中的登记还会显示本轮用时。
- `暂缓一轮` / `取消暂缓一轮`：调整当前等待登记的下一次游玩机会；固定组合会同时处理两份登记。
...
- `排队通知`：查看个人通知总开关和五个分项；可以分别调整队列状态、游玩位置、线上登记与签到、暂缓一轮与暂时离开及未到场、机台及营业状态通知。
...
通过 Bot 创建的是“线上登记”。玩家必须在创建登记后的 30 分钟内到终端点击自己的登记并选择“已到场”完成签到；超过 30 分钟，或轮到进入游玩位置时仍未签到，登记会自动退出排队。待签到期间不提供确定的等待时间估算，并且在 Bot 中只能退出排队；其他操作会由插件、服务器和终端分别校验并拒绝。
...
`pnpm pack` 会再次构建并生成 `koishi-plugin-maimai-q-0.3.4.tgz`。将该文件放到运行 Koishi 的机器，在 Koishi 项目根目录执行：

```bash
pnpm add -w /absolute/path/to/koishi-plugin-maimai-q-0.3.4.tgz
```
...
个人通知设置保存在玩家资料中，App、服务器和 Bot 使用同一份状态。总开关、队列状态、线上登记与签到、暂缓一轮/暂时离开/未到场默认开启；游玩位置、机台及营业状态默认关闭。通过 Bot 修改设置时，服务器会建立资料更新命令，现场终端确认并保存后才会生效。
