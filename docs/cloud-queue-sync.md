...
基础请求包含 `request_id`、`actor_qq` 和 `operation`。加入或切换机台时附带 `machine_id` / `target_machine_id`，修改本次偏好时附带 `preference`。确认请求还携带 `expected_queue_id` 和 `expected_machine_configuration_revision`。支持线上加入、退出、暂缓一次、暂时离开、取消对应状态、切换机台和修改本次游玩偏好。
